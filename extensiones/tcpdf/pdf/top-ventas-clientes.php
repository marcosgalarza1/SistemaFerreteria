...
class reporteTopVentasClientes
{
    ...
    public function generarPdfVentasTopClientes()
    {
        ...
        $respuestaVentas = ControladorVentas::ctrRangoFechasVentasTopClientesPdf($fechaInicio, $fechaFin);
        $itemUsuario = "id";
        ...
        $pdf->SetTitle('Top Ventas por cliente');
        $pdf->AddPage();

        $pdf->SetFont('helvetica', 'B', 11);
        $pdf->Cell(0, 5, 'REPORTE ENTRE FECHA', 0, 1, 'C');
        $pdf->Cell(0, 5, 'VENTAS POR CLIENTE', 0, 1, 'C');
        /* $pdf->Image('images/logo-negro-bloquee.jpg', 185, 10, 18, 16, 'jpg', '', 'T', false, 300, '', false, false, 0, false, false, false); */

        $pdf->Ln(10); // Espacio después de la imagen

        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->Cell(23, 5, 'Usuario/a: ', 0, 0, 'L');
        ...
        $pdf->Cell(0, 5, 'Detalle de Top Ventas por Cliente', 1, 1, 'C', 1);
        ...
        // Nro de compras
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->Cell(30, 5, 'Total Clientes:  ' . $contador - 1, 0, 0, 'L');
