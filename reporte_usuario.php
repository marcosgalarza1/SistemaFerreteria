...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 0, "REPORTE DE USUARIOS", 0, 1, "C");
    ...
    function TablaHeader()
    {
        ...
        $tablaAncho = 180; // Ancho total de la tabla en mm (210 mm - 2 * 10 mm de márgenes)
        ...
        $this->Cell($anchoColumna[3], 10, "ULTIMO LOGIN", 1, 1, "C", true);
    }
...
if ($resultado = $mysqli->query($sql)) {
    while ($fila = $resultado->fetch_assoc()) {
        ...
        $pdf->Cell($anchoColumna[1], 10, strtoupper($fila['usuario']), 1, 0, "L");
        $pdf->Cell($anchoColumna[2], 10, strtoupper($fila['perfil']), 1, 0, "L");
        $pdf->Cell($anchoColumna[3], 10, strtoupper($fila['ultimo_login']), 1, 1, "L");
    }
    $resultado->free();
} else {
